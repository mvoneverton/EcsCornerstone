/**
 * Generates /qa-checklist.md at the repo root — a manual QA checklist covering
 * every critical user flow, for pre-launch testing.
 *
 * Run:  npx ts-node packages/api/src/scripts/generateQAChecklist.ts
 *   or:  npm run qa:checklist --workspace=packages/api
 */
import { writeFileSync } from 'fs';
import path from 'path';

const REPO_ROOT = path.resolve(__dirname, '../../../../');
const OUT_PATH = path.join(REPO_ROOT, 'qa-checklist.md');

const CHECKLIST = `# ECS Cornerstone — Pre-Launch QA Checklist

_Generated ${new Date().toISOString()} by \`generateQAChecklist.ts\`. Re-run the script to regenerate; do not edit by hand._
...
writeFileSync(OUT_PATH, CHECKLIST, 'utf8');
console.log(`QA checklist written to ${OUT_PATH}`);
